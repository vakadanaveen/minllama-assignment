...
class LlamaEmbeddingClassifier(torch.nn.Module):
	...
	def forward(self, input_ids):
		...
		output_after_dropout = self.dropout(h)
		logits = self.classifier_head(output_after_dropout)
		log_softmax_results = F.log_softmax(logits, dim=-1)
		return log_softmax_results
